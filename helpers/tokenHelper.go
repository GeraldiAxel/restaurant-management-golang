...
func GenerateAllTokens(email string, firstName string, lastName string, uid string) (signedToken string, refreshToken string, err error) {
	...
	token, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims.StandardClaims).SignedString([]byte(SECRET_KEY))
	refreshToken, err = jwt.NewWithClaims(jwt.SigningMethodHS256, refreshClaims.StandardClaims).SignedString([]byte(SECRET_KEY))
