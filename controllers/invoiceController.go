...
func GetAllInvoices() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			msg := fmt.Sprintf("Failed to get invoices")
...
func GetInvoiceById() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("error: failed to get invoice")
		...
		invoiceViewFormat.Order_id = invoice.Order_id
		invoiceViewFormat.Payment_due_date = invoice.Payment_due_date
		invoiceViewFormat.Payment_method = "null"
		if invoice.Payment_method != nil {
			invoiceViewFormat.Payment_method = *invoice.Payment_method
		}
		invoiceViewFormat.Invoice_id = invoice.Invoice_id
		invoiceViewFormat.Payment_status = invoice.Payment_status
		invoiceViewFormat.Payment_due = allOrderItems[0]["payment_due"]
		invoiceViewFormat.Table_number = allOrderItems[0]["table_number"]
		invoiceViewFormat.Order_details = allOrderItems[0]["order_items"]
