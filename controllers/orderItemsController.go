...
func GetOrderItemById() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured fetching order item by id"})
			return
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	groupStage := bson.D{{"$group", bson.D{
		...
		{"total_count", bson.D{{"$sum", "$total_count"}}},
		{"table_number", bson.D{{"$first", "$table.table_number"}}},
